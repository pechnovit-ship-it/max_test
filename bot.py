...
def make_keyboard(rows):
    """Создаёт inline-клавиатуру. rows: список списков [(text, payload), ...]"""
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session_http:
            async with session_http.post(
                WEBHOOK_URL, json=report, headers=headers
            ) as resp:
                if resp.status == 200:
...
async def handle_attachments(event: MessageCreated):
    ...
    for attachment in attachments:
        if hasattr(attachment, "type") and attachment.type == "image":
            try:
                # Получаем URL изображения из вложения
                image_url = None
...
async def handle_text(event: MessageCreated):
    ...
    # Игнорируем команды — их обрабатывает CommandStart
    if text.startswith("/"):
        return
